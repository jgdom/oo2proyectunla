...
public class LecturaDao {
	...
	public Lectura traerLectura(long nroSerie, int mes) {
		...
		try {
			iniciaOperacion();
			String hql = "from Lectura l inner join fetch l.medidor m where m.nroSerie=" + nroSerie
	...
	public Lectura traerLectura(long nroSerie, int mes, int anio) {
		...
		try {
			iniciaOperacion();
			String hql = "from Lectura l inner join fetch l.medidor m where m.nroSerie=" + nroSerie
					+ "and month(l.fecha)=" + mes + "and year(l.fecha)="+anio;
			l = (Lectura) session.createQuery(hql).uniqueResult();
		} finally {
			session.close();
		}
	...
	public Lectura traerLectura(Medidor medidor, int mes, int anio) {
		...
		while (l == null && i < lista.size()) {
			if (Funciones.traerAnio(lista.get(i).getFecha()) == anio
